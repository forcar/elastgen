...
c     param: 1=dipole ff 2=Bosted ff 3=Brash ff
...
c     epsilon
      ...
      tau = q2/(4.*rm_p**2)
      eps = 1+2*(1+tau)*s2**2/c2**2
      eps = 1./eps
      
c     Reduced cross section

      sigr = eps*gep**2/tau+gmp**2
      sig = ((gep**2+tau*gmp**2)/(1.+tau))*c2**2+2.*tau*gmp**2*s2**2
...
c     Form factor
      ...
      real function elass(eb,theta,param)
...
c      jac=epr**2/pi
      th=acos(cx)*180./pi
      elasradq=elasrad(eb,th,t,0.,wcut,param)*jac
...
c     hep-ex\0111038 PRC 65, 051001
      ...
      real function elasrad(es,theta_d,izn,t1,t2,wcut,param)
...
c      print *, es,eel,del_mo,delta_s,delta_p1,delta_p2
